...
const extractedItemSchema = z.object({
  ...
  durationMax: z.number().int().min(5).max(720).default(30),
  ...
  flexibility: z.enum(["fixed", "flexible", "elastic"]).default("fixed"),
  constraints: z.array(z.string()).max(10).default([]),
  status: z.enum(["inbox", "scheduled"]).default("scheduled"),
  rawLabel: z.string().max(160).nullable().default(null),
  itemType: z.enum(["academic_subject", "assembly", "other"]).nullable().default(null),
  subjectConfidence: z.enum(["high", "medium", "low"]).nullable().default(null),
  subjectName: z.string().max(120).nullable().default(null),
  subjectShortName: z.string().max(30).nullable().default(null),
  teacher: z.string().max(120).nullable().default(null),
  room: z.string().max(80).nullable().default(null),
  evidence: z.string().min(1).max(240).default("Detected in the uploaded document."),
});
