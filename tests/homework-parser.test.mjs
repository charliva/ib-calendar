...
test("parses a review session locally, including duration and schedule", () => {
  const result = parseReviewSession(
    "Review Chemistry tomorrow at 16:30 for 50 min",
    subjects,
    now,
  );
  assert.deepEqual(result, {
    ...
    subjectId: "chemistry",
    durationMinutes: 50,
    startsAt: "2026-07-31T14:30:00.000Z",
    endsAt: "2026-07-31T15:20:00.000Z",
    deadline: null,
  });
});

test("recognizes specific homework without requiring a deadline", () => {
  const result = parseHomework("Biology worksheet", subjects, now);
  assert.equal(looksLikeHomeworkCommand(result.rawText, result), true);
});
