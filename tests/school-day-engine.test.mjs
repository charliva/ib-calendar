...
test("treats a 30-minute lesson changeover as unavailable", () => {
  const changeoverClasses = [
    {
      ...classes[0],
      startTime: "09:00",
      endTime: "10:00",
  ...
  const day = new Date("2026-07-31T08:00:00+02:00");
  assert.equal(
    detectFreePeriods(changeoverClasses, [], settings, day, day).length,
    0,
  );
});
...
test("free periods allocate distinct work before repeating a suggestion", () => {
  const periods = [
    {
      kind: "free_period",
      start: new Date("2026-07-31T11:25:00+02:00"),
      end: new Date("2026-07-31T12:25:00+02:00"),
      durationMinutes: 60,
    },
    {
      kind: "free_period",
      start: new Date("2026-08-07T11:25:00+02:00"),
      end: new Date("2026-08-07T12:25:00+02:00"),
      durationMinutes: 60,
    },
  ];
  const items = [
  ...
  const recommendations = recommendationsAcrossFreePeriods(
    periods,
    [],
    items,
    settings,
  ...
  assert.equal(
    recommendations.get(periods[0].start.toISOString())?.[0].sourceId,
    "biology-review",
  );
  assert.equal(
    recommendations.get(periods[1].start.toISOString())?.[0].sourceId,
    "tok-review",
  );
});
